...
export const googlePhotosUtils = {
  ...
  async getAlbumByName(albumTitle: string): Promise<string | null> {
    ...
    if (!listAlbumsResponse.ok) {
      throw new Error(
        `Failed to list albums: ${listAlbumsResponse.statusText}`,
      );
    }

    const listAlbumsResult = (await listAlbumsResponse.json()) as {
      albums: Album[];
    };
    const album = listAlbumsResult.albums.find(
  ...
  async createAlbumIfNotExists(albumTitle: string): Promise<string> {
    ...
    if (!createAlbumResponse.ok) {
      throw new Error(
        `Failed to create album: ${createAlbumResponse.statusText}`,
      );
    }

    const createAlbumResult = (await createAlbumResponse.json()) as {
      id: string;
    };
  ...
  async uploadImage(
    ...
    const newFileName = encodeURIComponent(
      googlePhotosUtils.generateGooglePhotosFileName(outputPath),
    );
    ...
    if (!createMediaItemResponse.ok) {
      throw new Error(
        `Failed to create media item: ${createMediaItemResponse.statusText}`,
      );
    }
